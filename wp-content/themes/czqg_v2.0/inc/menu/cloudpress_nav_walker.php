...
class CLOUDPRESS_nav_walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		{
			$CLOUDPRESS_drop_item='dropdown-item';
		}
		if ($args->has_children && $depth > 0) {
			$classes[] = 'dropdown';
			$CLOUDPRESS_drop_item='dropdown-item has-submenu';
...
function CLOUDPRESS_nav_menu_css_class( $classes ) {
	if ( in_array('current-menu-item', $classes ) OR in_array( 'current-menu-ancestor', $classes ) )
		$classes[]	=	'active';

	return $classes;
}
add_filter('nav_menu_css_class','CLOUDPRESS_nav_menu_css_class');
